...
---@field targetCell tes3cell? The find a path to. Mutually exclusive with `closestExterior`. This option is currently VERY slow. Probably many speed gains to be made here.
...
function tes3.tes3cell_findBestPath(self, params)
	...
	params.maxIterations = params.maxIterations or 10
	if params.limitToKnownCells == nil then
		params.limitToKnownCells = true
	end
	...
	--- A path of cells gets scored favorably if there are many connecting doors between the cells.
	--- Paths are also penalized based on their lenght.
	---@param path cellPath
	local function scorePath(path)
		...
		for i, count in ipairs(path.doorCounts) do
			...
			-- add tiny bonus based on number of activators
			score = score + path.cells[i + 1].activators.size / (256 * i)
		end
		path.score = score
		return score
	end
